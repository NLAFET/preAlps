...
int preAlps_tournamentPivotingCUR(int *xa, int *ja, double *a, int m,  int n,  int nnz, int k, long *Jr,
...
for(int dest = 1; dest<size; dest++) {
  MPI_Send(JJr,k,MPI_INT,dest,tag,MPI_COMM_WORLD);
}
...
/* Receiving QT matrix */
 ...
 /* Receiving Jr vector */
  MPI_Recv(JJr,k,MPI_INT,src,tag,MPI_COMM_WORLD,&stat);
...
if(rank>0) {
  ...
  MPI_Send(R->p,R->ncol+1,MPI_LONG,src,tag,MPI_COMM_WORLD);
  MPI_Send(R->i,R->nzmax,MPI_LONG,src,tag,MPI_COMM_WORLD);
  MPI_Send(R->x,R->nzmax,MPI_DOUBLE,src,tag,MPI_COMM_WORLD);

}
 else { // Master processor
 cholmod_sparse *tempVT;
 int nzmax = k*(A->ncol);
 tempVT = cholmod_l_allocate_sparse(k,A->ncol,nzmax,1,1,0,CHOLMOD_REAL,cc);
 /* Receiving local VT matrix from other processors */
  for(int dest = 1; dest<size;dest++) {
    MPI_Recv(tempVT->p,tempVT->ncol+1,MPI_LONG,dest,tag,MPI_COMM_WORLD,&stat);
    MPI_Recv(tempVT->i,tempVT->nzmax,MPI_LONG,dest,tag,MPI_COMM_WORLD,&stat);
    MPI_Recv(tempVT->x,tempVT->nzmax,MPI_DOUBLE,dest,tag,MPI_COMM_WORLD,&stat);
 ...
 tag = 1;
 /* Receiving local VT matrix from other processors and asembling the global R matrix */

 for(int dest = 1; dest<size;dest++) {
    MPI_Recv(tempVT->p,tempVT->ncol+1,MPI_LONG,dest,tag,MPI_COMM_WORLD,&stat);
    MPI_Recv(tempVT->i,tempVT->nzmax,MPI_LONG,dest,tag,MPI_COMM_WORLD,&stat);
    MPI_Recv(tempVT->x,tempVT->nzmax,MPI_DOUBLE,dest,tag,MPI_COMM_WORLD,&stat);
    R = cholmod_l_horzcat(R,tempVT,1,cc);
}
