                  ...
                  int          matMultVersion)
{
  ...
  for (int i = 0; i < pos->nval-1; ++i)
    if(pos->val[i+1] - pos->val[i]> maxRow) maxRow = pos->val[i+1] - pos->val[i];
  ...
  if(C_out->val == NULL)
  {
    ...
    ierr = CPLM_MatDenseCalloc(C_out);preAlps_checkError(ierr);
  }
  ...
  if (matMultVersion / 2 > 0)
  {
    ...
    for (int i = 0; i < nrecv; ++i)
    {
      ...
      ierr = CPLM_IVectorGetPos(dest,src,&srcIndest);preAlps_checkError(ierr);
